...
import type { AuthUser, RequestLike, ResponseLike } from './types';

const jwt = require('jsonwebtoken') as {
  sign(payload: AuthUser, secret: string, options: { expiresIn: string }): string;
...
export function readCookieToken(req: RequestLike): string | null {
...
function setAuthCookie(res: ResponseLike, token: string): void {
 ...
 */
export async function googleLogin(req: RequestLike<{ credential?: string }>, res: ResponseLike): Promise<ResponseLike> {
  try {
 ...
 */
export function getMe(req: RequestLike, res: ResponseLike): ResponseLike {
...
export function logout(_req: RequestLike, res: ResponseLike): ResponseLike {
  const isProd = process.env.NODE_ENV === 'production';
